...
parameters {
  ...
  matrix[n_site, n_year] eps_st;  // matrix of spatiotemporal random effects
}
transformed parameters {
  ...
  matrix[n_site, n_year] delta_st;  // appropriately scaled random effects (must construct)
...
model {
  ...
  for(t in 1:n_year){
    eps_st[,t] ~ multi_normal_cholesky(mu_zeros, L); 
  }
