...
-   For each distribution, will try provide its pmf (or pdf) expressed as a function of $y$ and the parameters of the distribution.
-   For example, will use $y\sim \mathrm{N}\left(\mu, \sigma^{2}\right)$ to indicate a random variable $Y$ is normally distributed with mean $\mu$ and variance $\sigma^2$
...
-   We release n = 50 and later count y = 6 sneakers
    -   successes vs. trials
...
my_data %>% 
    ...
    geom_vline(xintercept = MLE, linetype = 2, linewidth = 2, 
    color = "steelblue") + # add MLE
    theme_qfc()
```
...
my_data %>% 
    ...
    geom_point(size = 1.4) + 
    ylab("log-posterior") + xlab(expression(theta)) +
    geom_vline(xintercept = MAP, linetype = 2, linewidth = 2, 
               color = "steelblue") + # add MAP estimate
    theme_qfc()
```
...
```{R echo = T}
my_data %>% 
    ggplot(aes(x = theta, y = exp(logPost))) + # note exp() 
    geom_point(size = 1.4) + 
